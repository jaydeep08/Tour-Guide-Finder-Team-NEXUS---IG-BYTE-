...
def create_profile(request):
    ...
    personal_obj = Personal_Info(uuid=reg_data,name=name,mobile=mobile,charges=charges,available=available)
    personal_obj.save()

    for city in cities:
        city_obj = guide_city(uuid=reg_data,city_id=city)
        city_obj.save()

    for language in languages:
        language_obj = guide_language(uuid=reg_data,language_id=language)
        language_obj.save()

    for special in specilizations:
        spec_obj = guide_specilization(uuid=reg_data,spceilization_id=special)
        spec_obj.save()
